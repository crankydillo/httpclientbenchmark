...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dir = new File(
      //"/Users/sacox/work/httpclientbenchmark/asynchttpclient-benchmark/metrics-csv"
        args(0)
    )
    ...
    sections
      ...
      .foreach { case (method, types) =>
        println(s"<h1>$method</h1>")
        types.foreach { case (typ, collateralList) =>
          println(s"<h3>$typ</h3>")
